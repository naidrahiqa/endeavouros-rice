...
Singleton {
    ...
    readonly property MprisPlayer active: {
        ...
        // Multiple players can share an identity (e.g. brave + plasma-browser-integration).
        // Prefer the one exposing the richest metadata (art, artist, album, title).
        const matching = list.filter(p => getIdentity(p) === GlobalConfig.services.defaultPlayer);
        const pool = matching.length > 0 ? matching : list;
        let best = pool[0] ?? null;
        let bestScore = -1;
        for (const p of pool) {
            const score = (p.trackArtUrl ? 8 : 0) + (p.trackArtist ? 4 : 0) + (p.trackAlbum ? 2 : 0) + (p.trackTitle ? 1 : 0);
            if (score > bestScore) {
                bestScore = score;
                best = p;
            }
    ...
    function getArtUrl(player: MprisPlayer): string {
        ...
        if (url.match(/https:\/\/(www\.|music\.)?youtube\.com\/watch/)) {
            // Fallback for youtube / youtube music
