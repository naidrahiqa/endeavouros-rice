...
MouseArea {
    ...
    function save(): void {
        ...
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            ...
            const savePath = Paths.pictures + "/Screenshots/" + timestamp + ".png";
            Quickshell.execDetached(["sh", "-c", "mkdir -p " + Paths.pictures + "/Screenshots && cp " + path + " " + savePath]);
            Quickshell.execDetached(["notify-send", "-a", "caelestia-cli", "-i", path, "Screenshot taken", savePath]);
            closeAnim.start();
        });
    }
