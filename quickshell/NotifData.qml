...
QtObject {
    ...
    readonly property Connections conn: Connections {
        ...
        function onBodyChanged(): void {
            notif.body = notif.notification.body.replace(/<a\s+href="[^"]*">([^<]*)<\/a>/gi, "$1").replace(/<[^>]+>/g, "");
        }

        function onAppIconChanged(): void {
            const icon = notif.notification.appIcon;
            notif.appIcon = (icon.startsWith("file:///tmp/") || icon.includes("scoped_dir")) ? "" : icon;
        }
        ...
        function onImageChanged(): void {
            ...
            notif.image = (img.includes("scoped_dir") || (img.startsWith("image://icon//tmp/") && !img.startsWith("image://icon//home"))) ? "" : img;
    ...
    Component.onCompleted: {
        ...
        body = notification.body.replace(/<a\s+href="[^"]*">([^<]*)<\/a>/gi, "$1").replace(/<[^>]+>/g, "");
        const initIcon = notification.appIcon;
        appIcon = (initIcon.startsWith("file:///tmp/") || initIcon.includes("scoped_dir")) ? "" : initIcon;
        appName = notification.appName;
        const initImage = notification.image;
        image = (initImage.includes("scoped_dir") || (initImage.startsWith("image://icon//tmp/") && !initImage.startsWith("image://icon//home"))) ? "" : initImage;
