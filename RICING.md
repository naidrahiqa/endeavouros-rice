...
### Brave Notification Fix (NotifData.qml)
Brave/Chromium sends broken notification data on Wayland:
- `appIcon` points to temp files (`/tmp/org.chromium.Chromium.scoped_dir...`) that get cleaned up → purple/black broken icon
- `body` contains raw HTML (`<a href="...">`) which renders as literal text instead of links

**Patch:** `/etc/xdg/quickshell/caelestia/services/NotifData.qml`
- Strips HTML tags from `body` on `onBodyChanged` and `Component.onCompleted`
- Clears `appIcon` and `image` if they point to invalid temp paths (falls back to app name icon)
- Backup at `NotifData.qml.bak`
- **Warning:** Gets overwritten on Caelestia updates, re-apply after updating
